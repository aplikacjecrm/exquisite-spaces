...
export default function Navbar() {
  ...
  return (
    ...
    >
      {/* Top accent bar */}
      <div className="h-1 bg-gradient-to-r from-zinc-700 via-zinc-800 to-zinc-900" />
        ...
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <a href="#" className="flex items-center gap-3 group">
            ...
            <div className="hidden sm:flex flex-col leading-tight">
              <span className="text-zinc-900 font-black text-sm lg:text-base tracking-tight">EXQUISITE SPACES</span>
              <span className="text-zinc-400 font-mono text-[9px] lg:text-[10px] tracking-[0.2em] uppercase">Żywiec · Sp. z o.o.</span>
            </div>
          </a>
          ...
          <div className="hidden md:flex items-center gap-6 lg:gap-8">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-slate-600 hover:text-zinc-900 font-medium transition-colors text-sm relative group"
              >
                {link.label}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-zinc-900 group-hover:w-full transition-all duration-300" />
              </a>
            ))}
          </div>

          {/* Language switcher — desktop */}
          <div className="hidden md:flex items-center gap-1 border border-slate-200 rounded-lg p-0.5">
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className="hidden lg:flex items-center gap-2 text-slate-600 hover:text-zinc-900 text-sm font-medium transition-colors"
            >
              <Phone size={15} />
              +48 600 390 073
            </a>
            ...
            <button
              ...
              className="md:hidden p-2 text-slate-600 hover:text-zinc-900 rounded-lg hover:bg-slate-100 transition-colors"
              aria-label="Menu"
            >
